...
import { LayoutDashboard, Home, Pill, FileText, X, LogOut } from 'lucide-react'
...
function NurseNavItem({ to, icon: Icon, label }: NavItem) {
...
export function NurseSidebar({ mobileOpen, onClose }: Props) {
  ...
  const navItems: NavItem[] = [
    { to: '/nurse/dashboard', icon: LayoutDashboard, label: 'Bosh sahifa'      },
    { to: '/nurse/visit',     icon: Home,            label: 'Uy tashrifi'      },
    { to: '/nurse/meds',      icon: Pill,            label: 'Dori berish'      },
    { to: '/nurse/reports',   icon: FileText,        label: 'SSVR hisobotlari' },
  ]
  ...
  const sidebar = (
    <aside className="flex flex-col h-full w-[260px] bg-[var(--bg-primary)] border-r border-[var(--border-secondary)]">
      ...
      <div className="mx-3 mt-3 mb-1 p-3 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-secondary)]">
        <p className="text-[12px] font-semibold text-[var(--text-primary)] truncate">{user?.name ?? 'Hamshira'}</p>
        <p className="text-[11px] text-[var(--text-tertiary)] truncate mt-0.5">Patronaj hamshira · Oilaviy poliklinika #42</p>
      </div>

      <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-0.5">
        {navItems.map(item => (
          <NurseNavItem key={item.to} {...item} />
        ))}
      </nav>
